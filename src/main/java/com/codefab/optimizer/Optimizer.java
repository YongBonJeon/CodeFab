...
public class Optimizer implements Expr.Visitor<Expr>, Stmt.Visitor<Stmt> {
  ...
  public Expr visitGrouping(Expr.Grouping expr) {
    ...
    // A grouped literal needs no parentheses at runtime.
  ...
  /** Returns the folded value, or {@code null} when the operands are not foldable constants. */
  private Object foldBinary(Object left, TokenType op, Object right) {
    ...
    if (op == TokenType.EQUAL_EQUAL) return java.util.Objects.equals(left, right);
    if (op == TokenType.BANG_EQUAL) return !java.util.Objects.equals(left, right);
